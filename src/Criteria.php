 ...
 * - whitelist filtrů: [ 'policy_id', 'kms_key_id', 'weight', 'priority' ]
 * - whitelist LIKE:   []
 * Podporované tvary:
...
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        $allowed = array_fill_keys([ 'policy_id', 'kms_key_id', 'weight', 'priority' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'policy_id', 'kms_key_id', 'weight', 'priority' ], true);
            $parts = [];
