...
final class Definitions {
     ...
     * policy_id kms_key_id may be "id" or "col1, col2".
     * @return string[]
     */
    public static function pkColumns(): array {
        $raw = 'policy_id kms_key_id';
        ...
        if ($rawClean === '') {
            throw new \InvalidArgumentException('Definitions::pkColumns(): token policy_id kms_key_id must not be empty.');
        }
        return [$rawClean];
    }
    ...
    public static function defaultOrder(): ?string {
        $c = 'policy_id kms_key_id DESC'; return $c !== '' ? $c : null;
    }

    /** @return array<int,array<int,string>> list of unique keys */
    public static function uniqueKeys(): array { return [ [ 'policy_id', 'kms_key_id' ] ]; }
    ...
    public static function intColumns(): array { return [ 'policy_id', 'kms_key_id', 'weight', 'priority' ]; }
