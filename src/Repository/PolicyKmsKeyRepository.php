...
use BlackCat\Database\Packages\PolicyKmsKeys\Repository\PolicyKmsKeyRepositoryInterface;
...
class PolicyKmsKeyRepository implements PolicyKmsKeyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'policy_id', 'kms_key_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\PolicyKmsKeys\Definitions::class);
    ...
    public function getByPolicyIdAndKmsKeyId(int $policyId, int $kmsKeyId, bool $asDto = false): array|\BlackCat\Database\Packages\PolicyKmsKeys\Dto\PolicyKmsKeyDto|null {
        return $this->getByUnique([ 'policy_id' => $policyId, 'kms_key_id' => $kmsKeyId ], $asDto);
    }
    ...
    public function getIdByPolicyIdAndKmsKeyId(int $policyId, int $kmsKeyId) {
        $row = $this->getByPolicyIdAndKmsKeyId($policyId, $kmsKeyId, false);
        if (!is_array($row)) { return null; }
        return $row['policy_id kms_key_id'] ?? null;
    }
