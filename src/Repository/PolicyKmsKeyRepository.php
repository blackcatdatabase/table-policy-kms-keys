...
namespace BlackCat\Database\Packages\PolicyKmsKeys\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PolicyKmsKeys\Definitions;
use BlackCat\Database\Packages\PolicyKmsKeys\Criteria;
use BlackCat\Database\Packages\PolicyKmsKeys\Dto\PolicyKmsKeyDto as Dto;
use BlackCat\Database\Packages\PolicyKmsKeys\Mapper\PolicyKmsKeyDtoMapper as RowMapper;
...
final class PolicyKmsKeyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PolicyKmsKeys\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'policy_id', 'kms_key_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PolicyKmsKeys\Definitions::table());
            return $bulk->upsertMany($rows, [ 'policy_id', 'kms_key_id' ], [ 'weight', 'priority' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PolicyKmsKeys\Dto\PolicyKmsKeyDto|null */
    public function getByPolicyIdAndKmsKeyId(int $policyId, int $kmsKeyId, bool $asDto = false): array|\BlackCat\Database\Packages\PolicyKmsKeys\Dto\PolicyKmsKeyDto|null {
        $row = $this->getByUnique([ 'policy_id' => $policyId, 'kms_key_id' => $kmsKeyId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PolicyKmsKeys\Mapper\PolicyKmsKeyDtoMapper::fromRow($row);
    }
    public function existsByPolicyIdAndKmsKeyId(int $policyId, int $kmsKeyId): bool {
        $where = 't.' . Ident::q($this->db, 'policy_id') . ' = :uniq_policy_id' . ' AND ' . 't.' . Ident::q($this->db, 'kms_key_id') . ' = :uniq_kms_key_id';
        return $this->exists($where, [ 'uniq_policy_id' => $policyId, 'uniq_kms_key_id' => $kmsKeyId ]);
    }
    /** @return int|string|null */
    public function getIdByPolicyIdAndKmsKeyId(int $policyId, int $kmsKeyId) {
        $row = $this->getByPolicyIdAndKmsKeyId($policyId, $kmsKeyId);
        return $row ? ($row['policy_id kms_key_id'] ?? null) : null;
    }
